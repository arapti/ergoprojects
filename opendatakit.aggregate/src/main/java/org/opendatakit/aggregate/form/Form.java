...
public class Form implements IForm {
  ...
  private static FormGrantAuthority grantRow;
  ...
  Form(XFormParameters rootElementDefn, boolean isEncryptedForm, boolean isDownloadEnabled,
    ...
    {
    	logger.info("CREATING NEW FORM fileset");
      // get fileset (for now, zero or one record)
      FormInfoFilesetTable filesetRelation = FormInfoFilesetTable.assertRelation(cc);
      filesetRow = ds.createEntityUsingRelation(filesetRelation, user);
      filesetRow.setTopLevelAuri(topLevelAuri);
  ...
  public static void GrantForm(String formUri, CallingContext cc) throws ODKDatastoreException {
	  	Datastore ds = cc.getDatastore();
	    User user = cc.getCurrentUser();
	    Date now = new Date();
	  /*************************angeliki***************************/
	    logger.info("insert assignment of form in grant authority");
	    //after inserting form in form_info table we assign the form to site admin in form_granted
	    //authority table
	    FormGrantAuthority grantRelation=FormGrantAuthority.assertRelation(cc);
	    
	    grantRow = ds.createEntityUsingRelation(grantRelation, user);
	    //assign value for column _URI in form_granted_authority table
	    grantRow.setStringField(grantRow.primaryKey, formUri);
	    grantRow.setSubmissionDate(now);
	    grantRow.setMarkedAsCompleteDate(now);
	    grantRow.setIsComplete(true);
	    grantRow.setModelVersion(1L); 
	    grantRow.setUiVersion(0L);    
	    grantRow.setStringField(FormGrantAuthority.FORM_ID, formUri);
	    grantRow.setStringField(FormGrantAuthority.USER,user.getUriUser());
	   
	    
	   
	    ds.putEntity(grantRow, user);
  ...
  public synchronized void deleteForm(CallingContext cc) throws ODKDatastoreException {
    ...
    if (formDefinition != null) {
    	...
    	//create relation with table _form_granted authority 
        FormGrantAuthority relation2 = FormGrantAuthority
    			.assertRelation(cc);
        
        //make query(select) in table _form_granted authority 
        Query formAuthQuery = ds.createQuery(relation2,
    			"FormAuthority.getForms", user);
    	// filter forms according to form_id (URI) that matches the URI of the form that will be deleted
    	formAuthQuery.addFilter(FormGrantAuthority.FORM_ID,	Query.FilterOperation.EQUAL, infoRow.getUri());
    	List<? extends CommonFieldsBase> assocformRows = formAuthQuery
    			.executeQuery();
        
    	if ( assocformRows == null || assocformRows.size() == 0 ) {
    		return;
    	}

    	// delete the form data model...
    	List<EntityKey> grant = new ArrayList<EntityKey>();
    	//store the retrieved rows from the query in a list
        for ( CommonFieldsBase m : assocformRows ) {
        	grant.add(m.getEntityKey());
        }
        //delete all rows in the list 
        ds.deleteEntities(grant, user);
